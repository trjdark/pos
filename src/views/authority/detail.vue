    ...
    <div class="gym-page-authority">
        ...
        <div class="gym-page-authority-form">
            <div class="gym-page-authority-form-item">
                ...
                <div class="gym-page-authority-form-item-content">
                    <el-form :model="ruleForm" ref="ruleForm">
                        ...
                        >
                            <el-input v-model="ruleForm.roleCode" :disabled="true"></el-input>
                        </el-form-item>
                        <el-form-item
                            prop="roleName"
                            label="角色名称"
                ...
                <div class="gym-page-authority-form-item-content">
                    <template v-for="(item, index) in data"> 
                       <el-row class="menu-list">
                           <el-row>
                            <el-checkbox :key="item.menuId" v-model="item.checked === 1" @change="param => changeMenu(param, index)" :disabled="true">{{item.menuName}}</el-checkbox>
                        </el-row>
                        <el-row>
                             <el-checkbox v-for="(param ,k) in item.buttonMenus" :label="param.buttonId" :key="param.buttonId" v-model="param.checked === 1" @change="param => changeBtn(param, index, k)" :disabled="true">{{param.buttonName}}</el-checkbox> 
                        </el-row>
                       </el-row>
                    </template>
                </div>
            ...
            <div class="gym-page-authority-form-item">
                ...
                <div class="gym-page-authority-form-item-content">
                    <template >   
                        <el-checkbox :indeterminate="isIndeterminate" v-model="checkAllArea" @change="handleCheckAllArea" :disabled="true">ALL</el-checkbox>
                        <!-- <template v-for="secondItem in item.buttonMenus"> -->
                        <el-checkbox-group v-model="checkArea" @change="handleCheckArea" :disabled="true">
                            <el-checkbox v-for="(item, index) in area"  class="el-group"
                                :index="index" :key="item.id" :label="item.id">{{item.areaName}}
                            </el-checkbox>
                        </el-checkbox-group> 
                    </template>
                </div>
            ...
            <div class="gym-page-authority-form-item">
                ...
                <div class="gym-page-authority-form-item-content">
                    <el-button size="small" @click="editRole" :disabled="true">
                        角色人员分配
                    </el-button>
...
<script lang="ts">
    ...
    export default class AuthorityDetail extends Vue {
        public titles = [
            {
                name: '权限管理',
                link: '',
            ...
            Promise.all([
                ...
                authorityDetail({roleId: this.id}).then((res:any) => {
                    ...
                    res.checkedMenuAuthList.map((item:any) => {
                        ...
                        const a = checked === 1 && ids.push(menuId);
                        const key:any = 'buttonMenus';
                        const b = item[key] && item[key].forEach((param:any) => {
                            const c = param.checked === 1 && ids.push(param.buttonId);
                        });
                    });
                    ids = [...new Set(ids)];
                    this.data.forEach((element:any) => {
                        ...
                        const key:any = 'buttonMenus';
                        const e = element[key] && element[key].forEach((k:any) => {
                            const f = this.$set(k, 'checked', ids.includes(k.buttonId) ? 1: 0);
                        });
                    });
